...
(function installClickDelegate() {
  ...
  document.addEventListener('click', function globalOnClickHandler(e) {
    ...
    try {
      ...
      console.error('Error delegado onclick (root):', fnPath, err);
    }
  }, { capture: true });
})();
...
const CHAMBEADOR_DEFS = [
  ...
  { key: "week",   elId: "dest-semana", inTime: "chambeador-week-reveal",   inHours: "chambeador-week-hours",   inMinutes: "chambeador-week-minutes" },
  { key: "month",  elId: "dest-mes",    inTime: "chambeador-month-reveal",  inHours: "chambeador-month-hours",  inMinutes: "chambeador-month-minutes" }
];

function chambeadorDefaults() {
  return {
    day:  { revealTime: "12:00", visibleHours: 2,  visibleMinutes: 0 },
    week: { revealTime: "18:00", visibleHours: 72, visibleMinutes: 0 },
    month:{ revealTime: "20:00", visibleHours: 168, visibleMinutes: 0 }
  };
}

function parsePeriodConfig(raw, key) {
  const dflt = chambeadorDefaults()[key];
  const d = (raw && typeof raw === "object") ? raw : {};
  return {
    revealTime: (d.revealTime && /^\d{2}:\d{2}$/.test(String(d.revealTime)))
      ? String(d.revealTime)
      : (dflt.revealTime || "12:00"),
    visibleHours: Number.isFinite(Number(d.visibleHours)) ? Number(d.visibleHours) : dflt.visibleHours,
    visibleMinutes: Number.isFinite(Number(d.visibleMinutes)) ? Number(d.visibleMinutes) : dflt.visibleMinutes
  };
}

async function loadChambeadorConfig() {
  try {
    const snap = await getDoc(doc(db, "settings", CHAMBEADOR_CONFIG_DOC));
    const data = snap.exists() ? snap.data() : {};
    for (const def of CHAMBEADOR_DEFS) {
      const cfg = parsePeriodConfig(data[def.key], def.key);
      const elT = document.getElementById(def.inTime);
      const elH = document.getElementById(def.inHours);
      const elM = document.getElementById(def.inMinutes);
      if (elT) elT.value = cfg.revealTime;
      if (elH) elH.value = cfg.visibleHours;
...
window.saveChambeadorConfig = async () => {
  ...
  try {
    const config = {};
    for (const def of CHAMBEADOR_DEFS) {
      const revealTime = document.getElementById(def.inTime)?.value || "12:00";
      const visibleHours = parseInt(document.getElementById(def.inHours)?.value, 10);
      const visibleMinutes = parseInt(document.getElementById(def.inMinutes)?.value, 10);
...
function startChambeadorScheduler() {
  if (chambeadorSchedulerTimer) {
    ...
    chambeadorSchedulerTimer = null;
  }
  chambeadorSchedulerTimer = setInterval(applyChambeadorVisibility, 30 * 1000); // cada 30 s
  console.log("[Chambeadores] Scheduler iniciado: revisa visibilidad cada 30s");
...
function periodWindowToday(periodCfg) {
  ...
  const revealTs = new Date(now.getFullYear(), now.getMonth(), now.getDate(), parts[0], parts[1], 0, 0).getTime();
  const durationMs = ((Number(periodCfg.visibleHours) || 0) * 60 + (Number(periodCfg.visibleMinutes) || 0)) * 60 * 1000;
  const hideTs = revealTs + durationMs;
  return { revealTs, hideTs };
}

async function applyChambeadorVisibility() {
  try {
    const snap = await getDoc(doc(db, "settings", CHAMBEADOR_CONFIG_DOC));
    const data = snap.exists() ? snap.data() : {};
    const config = {};
    ...
    for (const def of CHAMBEADOR_DEFS) {
      ...
      config[def.key] = parsePeriodConfig(stored, def.key);
      if (stored && typeof stored === "object") hasStored = true;
    }
    ...
    if (!hasStored) {
      ...
      for (const def of CHAMBEADOR_DEFS) {
        ...
        const elH = document.getElementById(def.inHours);
        const elM = document.getElementById(def.inMinutes);
        if (elT && elT.value) config[def.key].revealTime = elT.value;
        if (elH && elH.value) config[def.key].visibleHours = Number(elH.value);
        if (elM && elM.value) config[def.key].visibleMinutes = Number(elM.value);
      }
    }
    ...
    for (const def of CHAMBEADOR_DEFS) {
      ...
      if (!el) continue;
      const win = periodWindowToday(cfg);
      let visible = false;
      if (win) {
        const durationMs = win.hideTs - win.revealTs;
        const nextReveal = win.revealTs + 24 * 60 * 60 * 1000;
        const inWindow = nowMs >= win.revealTs && nowMs < win.hideTs;
        const inNextWindow = nowMs >= nextReveal && nowMs < nextReveal + durationMs;
    ...
    console.error("Error aplicando visibilidad de chambeadores:", e);
  }
}
...
function weekKeyOf(dt) {
  ...
  const ms = Date.UTC(d.getFullYear(), d.getMonth(), d.getDate()) - dayN * 86400000;
  const start = new Date(ms);
  const y = start.getUTCFullYear();
  const firstThursday = new Date(Date.UTC(y, 0, 4));
  const week1Monday = new Date(firstThursday.getTime() - (((firstThursday.getUTCDay() + 6) % 7) * 86400000));
  const week = Math.ceil((((start.getTime() - week1Monday.getTime()) / 86400000) + 1) / 7);
  return `${y}-W${String(Math.max(1, week)).padStart(2, "0")}`;
}

function monthKeyOf(dt) {
  const d = new Date(dt);
  return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}`;
}

// Guarda un punto de control del historial con addDoc (nunca pisa registros previos).
async function writePointSnapshot(uid, points, reason, clientTs) {
  try {
    const t = Number.isFinite(Number(clientTs)) ? Number(clientTs) : Date.now();
    const dt = new Date(t);
    await addDoc(collection(db, "pointSnapshots"), {
      uid,
      points: Number(points),
      reason: reason || "",
      clientTs: t,
      dayKey: dayKey(dt),
      weekKey: weekKeyOf(dt),
      monthKey: monthKeyOf(dt),
      createdAt: serverTimestamp()
    });
  } catch(e) {
    console.error("Error guardando snapshot de puntos:", e);
  }
}
...
await writePointSnapshot(uid, nv, reason, Date.now());
...
window.renderEvolutionPts = () => {
  ...
  // Incluir TODOS los roles (Usuario, Admin e Inspector) sin omitir ni agrupar.
  let team = allUsers.filter(u => ["admin", "inspector", "user"].includes(String(u.role || "").toLowerCase()));
  ...
  if (!team.length) {
    el.innerHTML = '<div class="chart-empty">Sin miembros que coincidan con los filtros.</div>';
  ...
  const stepMs = isDayView ? 60 * 60 * 1000 : 24 * 60 * 60 * 1000;
  const nowMs = Date.now();
  ...
  const getUserColor = (uid) => {
    ...
    userColorCache[uid] = PALETTE[Math.abs(hash) % PALETTE.length];
  ...
  const series = team.map((m) => {
    ...
    for (const l of logs) {
      if (!l || l.type !== "points" || l.targetUid !== m.uid) continue;
      const t = logTimeRaw(l);
      if (!t) continue;
      const bi = periods.findIndex(d => t >= d.start && t < d.end);
      if (bi >= 0) periodChanges[bi] += typeof l.delta === "number" ? l.delta : 0;
    }
    let val = Number(m.points || 0);
    const factor = Math.pow(10, decimalsN());
    const values = new Array(count).fill(0);
    for (let i = count - 1; i >= 0; i--) {
      values[i] = Math.round(val * factor) / factor;
      val -= periodChanges[i];
    }
    return { uid: m.uid, name: cleanName(m.name), color: getUserColor(m.uid), values };
  });

  const mode = modeStates.evo || "line";
  const W = 900, H = 430, pl = 42, pr = 16, pt = 22, pb = 42;
  const iw = W - pl - pr, ih = H - pt - pb;
  ...
  if (mode === "line") {
    ...
    series.forEach((s) => {
      ...
      s.values.forEach((v, i) => {
        content += `<circle cx="${xPos(i)}" cy="${yPos(v)}" r="2.5" fill="${s.color}"/>`;
      });
    });
  } else {
    ...
    const barGap = Math.max(0.5, (groupWidth - groupGap - (barWidth * userCount)) / (userCount + 1));
    const bsX = (pi, ui) => pl + (pi * groupWidth) + groupGap / 2 + barGap + ui * (barWidth + barGap);
    series.forEach((s, si) => {
      s.values.forEach((v, i) => {
        const hBar = (v / maxVal) * ih;
        const y = pt + ih - hBar;
        content += `<rect x="${bsX(i, si)}" y="${y}" width="${barWidth}" height="${Math.max(0.5, hBar)}" fill="${s.color}" rx="1.5"/>`;
      });
    });
    // Con muchos usuarios se fuerza scroll horizontal (no se aplastan las barras).
    svgMinWidth = Math.max(W, Math.ceil(n * (barWidth * userCount + barGap * (userCount + 1) + groupGap)));
  }

  if (n > 12) {
    for (let i = 0; i < n; i += 2) {
      xl += `<text x="${xPos(i)}" y="${H - 8}" text-anchor="middle" font-size="8" fill="#7c86ad">${periods[i].label}</text>`;
    }
  } else {
    periods.forEach((p, i) => {
      xl += `<text x="${xPos(i)}" y="${H - 8}" text-anchor="middle" font-size="9" fill="#7c86ad">${p.label}</text>`;
    });
  }
...
window.renderActivityChart = () => {
  ...
  // Estructura para almacenar actividad por rol (sin doble conteo por log).
  const roleActivity = {
  ...
  for (const l of logs) {
    ...
    if (filterState.rol && actorRole !== filterState.rol) continue;
  ...
  for (const [bi, mapActors] of perBucketActors) {
    ...
    for (const [uid, rec] of mapActors) {
      ...
      if (rec.role === "user") {
        ...
        inspV += Number(actor?.points || 0) + rec.added + rec.logins;
      } else if (rec.role === "admin") {
        // Admin: puntos que agrega + entradas.
        adminV += rec.added + rec.logins;
      }
    }
    roleActivity.user[bi] = userV;
    roleActivity.inspector[bi] = inspV;
    roleActivity.admin[bi] = adminV;
    maxVal = Math.max(maxVal, userV, inspV, adminV);
  }
